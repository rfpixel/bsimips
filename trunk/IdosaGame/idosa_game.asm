...
<<<<<<< .mine
# 18/04/13
# pequenas correções e acertos ortográficos
# o que faltava, foi implementado na versão anterior mesmo
#
# 11/4/13 23:55 
...
#INÃCIO DO PROGRAMA!
	...
	li $k1, 0
	#Zera a pontuaÃ§ao do jogador O
	li $k0, 0
...
tecla19:	li $v0,5 #tecla de 1 atÃ© 9
		syscall
...
#INÍCIO DO PROGRAMA!
	...
	li $t1, 2 #INICIALIZA AS POSICOES O VALOR 2 (NEM X, NEM O) 
	li $t2, 2
	li $t3, 2
	li $t4, 2
	li $t5, 2
...
tecla19:	
	...
	# Verifica se é a vez do X ou do O. (se contador $a3 for impar vez do X, 
	# se for par vez do 0)
	andi $s1, $a3, 1
...
volta_alguem_ganhou:
	...
	j volta_ao_principal    # rotina tecla19 
...
pulo_ob:	
	...
	add	$s5,$s5,$s4
	add	$s5,$s5,$s4
	#desenho os pontos depois do espacamento
	sw 	$a2, bitmap_address($s5)
	addi 	$s5, $s5, 4 
	sw 	$a2, bitmap_address($s5)	
...
<<<<<<< .mine
	...
	addi $s2, $s2, 256	#passa para linha debaixo 256 = 64(posicoes) * 4 (bytes por 
				# posicao ocupados no BitmapDisplay)
...
pulo147_c:
	sw $a2, bitmap_address($s2) #comando que desenha pixel no Bitmap Display
	addi $s2, $s2, 256	#passa para linha debaixo 256 = 64(posicoes) * 4 (bytes 
                                #por posicao ocupados no BitmapDisplay)
	subi $s3, $s3, 1	#decrementa contador
	beq $s3, $zero, fim147_c   #testa fim
	j pulo147_c
fim147_c:
